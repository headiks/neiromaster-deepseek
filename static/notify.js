 ...
 *
 * Опрашивает инбокс (/api/my/messages) и показывает НОВЫЕ сообщения ПО ОДНОМУ:
 * всплывашка в углу + браузерное уведомление. Если накопилось несколько — это
 * очередь: первое показывается, затем через задержку следующее.
 *
 * Мультиустройство: «показано» отслеживается ПО УСТРОЙСТВУ (набор seen в памяти
 * этого сеанса), а НЕ по глобальному статусу read. Поэтому если тем же аккаунтом
 * пользуются с 2+ устройств, уведомление всплывает на КАЖДОМ из них, даже если
 * другое устройство успело открыть/пометить сообщение прочитанным раньше нашего
 * опроса (тогда сервер отдаёт его уже как 'read', но для нас оно всё ещё новое).
 * На первом опросе снимаем слепок инбокса, чтобы не всплывала прочитанная история.
 * Когда всплывашка закрыта (кнопкой или сама по таймеру), сообщение помечается
 * прочитанным на сервере (глобально, на аккаунт) и очередь двигается дальше.
 *
...
(function () {
  ...
  var seen = {};           // id -> true: уже в очереди или показаны на ЭТОМ устройстве
  var primed = false;      // сделан ли первый опрос (снят слепок инбокса)
    ...
    api("/api/my/messages")
      ...
      .then(function (d) {
        (d.messages || []).forEach(function (m) {
          if (seen[m.id]) return;              // на этом устройстве уже показано
          seen[m.id] = true;
          // Первый опрос — только слепок: всплываем лишь по ещё не прочитанным
          // (delivered), чтобы не сыпать историей. Дальше — по любому новому id
          // независимо от статуса (другое устройство могло уже пометить read).
          if (primed || m.status === "delivered") {
