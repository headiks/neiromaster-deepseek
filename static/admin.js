...
// refreshIcons() определяется отдельным inline-скриптом после загрузки Lucide (см. admin.html).
            ...
            apiJson('/staffing/import', {
                ...
                document.getElementById('staffing-result').innerHTML = `
                    ${profiles.length ? `<div class="plan-note" style="margin:12px 0;"><i data-lucide="download"></i>
                        Excel с логинами и паролями скачан. Повторно — кнопкой «Логины и пароли (Excel)».
                        Пароль сотрудника меняет только администратор (кнопка «Доступ»).</div>` : ''}
        ...
        function loginCell(e) {
            ...
            const tmp = e.temp_password
                ? ` · пароль: <code class="mono">${escapeHtml(e.temp_password)}</code>` : '';
        ...
        async function saveEmployee() {
            ...
            closeEmployeeDialog();
            if (!id) {
                alert(`Сотрудник создан.\nЛогин: ${data.username}\nПароль: ${data.temp_password}\n\n`
                    + 'Пароль виден в списке и в Excel. Сменить его можно кнопкой «Доступ».');
            }
            loadEmployees();
        }
        ...
        function submitCredentials() {
            ...
            apiJson(`/users/${encodeURIComponent(id)}/credentials`, {
                ...
                res.innerHTML = `Логин: <b>${escapeHtml(data.username)}</b> · пароль: <code class="mono">${escapeHtml(data.temp_password)}</code>`;
